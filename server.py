...
def data():  # listens to the data streamed from the sensor logger
	if str(request.method) == "POST":
		...
		ts = int(datetime.fromisoformat(data["loggingTime"]).timestamp())
		if len(time) == 0 or ts > time[-1]:
			time.append(ts)
			# modify the following based on which sensor is accessed, log the raw json for guidance
			accel_x.append(data["motionUserAccelerationX"])
			accel_y.append(data["motionUserAccelerationY"])
			accel_z.append(data["motionUserAccelerationZ"])
	return "success"
